...
export async function addToPublicFeed(hostname, score, grade) {
  try { await db.execute({ sql: `INSERT INTO public_feed (hostname, score, grade, scanned_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)`, args: [hostname, score, grade] });
  } catch {
    try { await db.execute({ sql: `INSERT INTO public_feed (hostname, score, grade, created_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)`, args: [hostname, score, grade] });
    } catch { await db.execute({ sql: `INSERT INTO public_feed (hostname, score, grade) VALUES (?, ?, ?)`, args: [hostname, score, grade] }); }
  }
}
...
export async function addMonitor({ userId, hostname, interval = "weekly", threshold = 70 }) { await db.execute({ sql: `INSERT INTO monitors (user_id, hostname, check_interval, alert_threshold, next_run_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(user_id, hostname) DO UPDATE SET check_interval = excluded.check_interval, alert_threshold = excluded.alert_threshold, is_active = 1`, args: [userId, hostname.toLowerCase(), interval, threshold, Date.now()] }); }

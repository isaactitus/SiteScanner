  ...
  try {
    ...
    try {
      ...
      if (response.ok) { 
        ...
        // Capture the real error from Google (e.g. "API key not valid")
        aiError = `Google API Error: ${errorData.error?.message || response.statusText}`; 
      }
    } catch (err) { 
      aiError = `Network Error: ${err.message}`; 
    }

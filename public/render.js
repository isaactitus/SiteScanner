...
window.loadRecentFeed = async function() {
  ...
  } catch { recentListEl.innerHTML = '<span style="color:var(--text-tertiary); font-size:0.8rem;">System offline</span>'; }
};
...
window.showDnsVerificationModal = async function(hostname, onSuccess) {
  ...
  const modalHtml = `
    <div id="dnsModal" style="position:fixed; inset:0; background:rgba(10, 14, 23, 0.85); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:center; z-index:9999; padding:20px; overflow-y: auto; animation: fadeIn 0.3s ease;">
  ...
  if (!isGuest) {
              ...
              if (aiRes.aiReport) {
                aiHtml = `
                  ...
                  <div class="action-grid" style="grid-template-columns: repeat(2, 1fr); margin-top: 24px; animation: floatUpFade 0.8s var(--ease-float);">
                    <button onclick="document.getElementById('exportPdfBtn').click()" class="cta-button" type="button" style="background: transparent;">Export Output to PDF</button>
    ...
    document.getElementById("monitorBtn")?.addEventListener("click", async () => {
      ...
      if (isMonitoring) {
        ...
        const modalHtml = `
          <div id="disableModal" style="position:fixed; inset:0; background:rgba(10, 14, 23, 0.85); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:center; z-index:9999; padding:20px; animation: fadeIn 0.3s ease;">
...
window.showLoadingState = function(container, mode) {
  ...
  const color = isActive ? 'var(--brand-emerald)' : 'var(--text-primary)';
